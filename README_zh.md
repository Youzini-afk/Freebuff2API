...
- **管理后台 WebUI** — 内置 `/admin` 控制台，可在线管理 Token、查看健康度与流量统计。
...
| 属性 / 环境变量 | 说明 |
|---|---|
| `LISTEN_ADDR` | 代理监听地址（默认 `:8080`，未设置时会自动读取平台注入的 `PORT`） |
| `UPSTREAM_BASE_URL` | Freebuff 后端地址（默认 `https://codebuff.com`） |
| `AUTH_TOKENS` | Freebuff Auth Token，用作首次启动时的种子；启用管理后台后以持久化存储为准 |
| `ROTATION_INTERVAL` | Run 自动轮换间隔（默认 `6h`） |
| `REQUEST_TIMEOUT` | 上游请求超时时间（默认 `15m`） |
| `API_KEYS` | 客户端鉴权 API Key（留空则无需鉴权，不影响 `/admin`） |
| `HTTP_PROXY` | 上游 HTTP 代理地址 |
| `DATA_DIR` | 持久化目录，用于保存 Token 和流量统计（容器默认 `/data`） |
| `ADMIN_PASSWORD` | 管理后台登录密码；留空则禁用 `/admin` 路由 |

同时设置时，环境变量优先于 JSON 配置文件。

## 管理后台 WebUI

设置 `ADMIN_PASSWORD` 后，访问 `http://<服务地址>/admin` 即可进入后台控制台：

- **概览** — 累计请求、最近 1m / 5m / 1h / 24h 流量、平均延迟、Token 健康度。
- **Tokens** — 在线新增 / 重命名 / 启用 / 禁用 / 删除 Token，显示每个 Token 的会话状态、冷却时间和最近错误。
- **流量统计** — 最近 1h / 6h / 24h 的请求曲线、按 Token 的流量排行。
- **设置** — 查看运行时配置与暴露的 API 端点。

所有 Token 与累计流量数据保存在 `${DATA_DIR}/tokens.json` 与 `${DATA_DIR}/metrics.json`，重启后依旧可用。

> 会话使用签名 Cookie（`SameSite=Lax`），服务重启后需要重新登录。管理后台的请求不会使用 `API_KEYS` 客户端鉴权。
...
镜像默认将 `DATA_DIR` 设置为 `/data`，只要挂上卷，Token 与流量统计就会在容器重启后保留。

### Zeabur 部署

1. 在 Zeabur 创建服务并选择本仓库，构建方式选 **Dockerfile**。
2. 在环境变量中填入：
   - `AUTH_TOKENS`（可选，用作首次启动时的种子 Token，可留空并完全通过 WebUI 管理）
   - `ADMIN_PASSWORD`（启用管理后台 WebUI）
   - 如需对外部调用方鉴权，可同时设置 `API_KEYS`
3. **不要**手动设置 `LISTEN_ADDR`，服务会自动使用 Zeabur 注入的 `PORT`。
4. 在 Zeabur 为服务挂一个持久化卷到 `/data`，以便 Token 与流量统计在重启后保留。
5. 健康检查路径填 `/healthz`；部署完成后访问 `https://<你的域名>/admin` 即可登录。
